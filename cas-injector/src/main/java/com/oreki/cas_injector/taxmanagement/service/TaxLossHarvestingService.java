...
public class TaxLossHarvestingService {
    ...
    public List<TlhOpportunity> scanForOpportunities(String investorPan) {
        ...
        // 2. Group by AMFI Code
        Map<String, List<TaxLot>> lotsByAmfi = allOpenLots.stream()
        ...
        lotsByAmfi.keySet().forEach(amfi -> {
            double nav = amfiService.getLatestSchemeDetails(amfi).getNav().doubleValue();
            navCache.put(amfi, nav);
        });

        for (Map.Entry<String, List<TaxLot>> entry : lotsByAmfi.entrySet()) {
            String amfi = entry.getKey();
            List<TaxLot> fundLots = entry.getValue();
            Scheme scheme = fundLots.get(0).getScheme();
            
            // 3. Sort lots strictly by Buy Date ASC (The FIFO Law)
            List<TaxLot> fifoQueue = fundLots.stream()
                .sorted(Comparator.comparing(TaxLot::getBuyDate))
                .toList();

            double currentNav = navCache.getOrDefault(amfi, 0.0);
            ...
            for (TaxLot lot : fifoQueue) {
                ...
                double lotLoss = (lotCost * lot.getRemainingUnits().doubleValue()) - lotValue; 
            ...
            if (accumCapitalLoss >= MIN_ABSOLUTE_LOSS_THRESHOLD) {
                ...
                String proxy = proxyMap.getOrDefault(amfi, "Search for similar category peer");
        ...
        for (Map.Entry<String, List<TaxLot>> entry : lotsByAmfi.entrySet()) {
            ...
            String proxyFundName = proxyMap.get(amfi);
            if (proxyFundName == null) continue;

            double totalCost = fundLots.stream().mapToDouble(l -> l.getCostBasisPerUnit().doubleValue() * l.getRemainingUnits().doubleValue()).sum();
            double currentNav = navCache.getOrDefault(amfi, 0.0);
            if (currentNav <= 0) continue;
